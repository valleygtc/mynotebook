...
function initDB() {
  ...
  const stmt = db.prepare(
    `insert into section (id, sequence, title)
  ...
  const stmt2 = db.prepare(
    `insert into page (id, section_id, level, sequence, title, content)
...
function addSection(sequence, title) {
  const id = db.prepare(
    `insert into section (sequence, title)
           values (?, ?)`
  ).run([sequence, title]);
  return id;
}
...
function addPage(sectionId, level, sequence, title, content) {
  const id = db.prepare(
    `insert into page (section_id, level, sequence, title, content)
       values (?, ?, ?, ?, ?)`
  ).run([sectionId, level, sequence, title, content]);
  return id;
}
